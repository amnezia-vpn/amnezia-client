...
bool WindowsFirewall::init() {
    ...
    subLayer.displayData.name = (PWSTR) L"ZloVPN-SplitTunnel-Sublayer";
...
#define FW_OK(rule)                                                                                                    \
    ...
    FW_OK(allowTrafficForAppOnAll(getCurrentPath(), MAX_WEIGHT, "Allow all for ZloVPN.exe"));
...
bool WindowsFirewall::enablePeerTraffic(const InterfaceConfig &config) {
    ...
    if (!config.m_dnsServer.isEmpty()) {
        if (!allowTrafficTo(QHostAddress(config.m_dnsServer), 53, HIGH_WEIGHT, "Allow DNS-Server",
    ...
    if (!config.m_excludedAddresses.empty()) {
        for (const QString &i: config.m_excludedAddresses) {
            ...
            if (!allowTrafficToRange(i, HIGH_WEIGHT, "Allow Ecxlude route", config.m_serverPublicKey)) {
                return false;
            }
        }
    }
...
bool WindowsFirewall::allowTrafficToRange(const IPAddress &addr, uint8_t weight, const QString &title,
                                          const QString &peer) {
