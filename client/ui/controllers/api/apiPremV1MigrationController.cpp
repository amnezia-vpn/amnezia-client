...
ApiPremV1MigrationController::ApiPremV1MigrationController(ServersController* serversController,
                                                           ...
                                                           QObject *parent)
    : QObject(parent), m_serversController(serversController), m_serversModel(serversModel), m_settingsController(settingsController)
{
}
...
bool ApiPremV1MigrationController::hasConfigsToMigration()
{
    ...
    if (!vpnKeys.isEmpty()) {
        GatewayController gatewayController(m_settingsController->getGatewayEndpoint(), m_settingsController->isDevGatewayEnv(), apiDefs::requestTimeoutMsecs,
                                            m_settingsController->isStrictKillSwitchEnabled());
...
{
    return m_settingsController->isPremV1MigrationReminderActive();
}

void ApiPremV1MigrationController::disablePremV1MigrationReminder()
{
    m_settingsController->disablePremV1MigrationReminder();
}
