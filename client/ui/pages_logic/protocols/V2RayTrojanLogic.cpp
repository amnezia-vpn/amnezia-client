...
V2RayTrojanLogic::V2RayTrojanLogic(UiLogic *logic, QObject *parent):
...
void V2RayTrojanLogic::updateProtocolPage(const QJsonObject &v2RayConfig, DockerContainer container, bool haveAuthData)
{
    set_pageEnabled(haveAuthData);
    set_pushButtonSaveVisible(haveAuthData);
    set_progressBarResetVisible(haveAuthData);

    set_lineEditServerPortText(v2RayConfig.value(config_key::port).toString(protocols::v2ray_trojan::defaultServerPort));
    set_lineEditServerPortEnabled(container == DockerContainer::V2RayTrojan);

    set_lineEditLocalPortText(v2RayConfig.value(config_key::local_port).toString(protocols::v2ray_trojan::defaultLocalPort));
    set_lineEditLocalPortEnabled(container == DockerContainer::V2RayTrojan);

}

QJsonObject V2RayTrojanLogic::getProtocolConfigFromPage(QJsonObject oldConfig)
{
...
void V2RayTrojanLogic::onPushButtonSaveClicked()
{
    QJsonObject protocolConfig = m_settings->protocolConfig(uiLogic()->m_selectedServerIndex, uiLogic()->m_selectedDockerContainer, Proto::V2RayTrojan);
    ...
    newContainerConfig.insert(ProtocolProps::protoToString(Proto::V2RayTrojan), protocolConfig);
...
void V2RayTrojanLogic::onPushButtonCancelClicked()
{
