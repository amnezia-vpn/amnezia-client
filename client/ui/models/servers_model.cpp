...
ServersModel::ServersModel(std::shared_ptr<Settings> settings, QObject *parent) :
    ...
    connect(this, &ServersModel::defaultServerIndexChanged, this, [this](const int serverIndex) {
        auto defaultContainer = ContainerProps::containerFromString(
                m_servers.at(serverIndex).toObject().value(config_key::defaultContainer).toString());
...
const int ServersModel::getDefaultServerIndex() { return m_defaultServerIndex; }
...
const QString ServersModel::getDefaultServerDescriptionCollapsed() {
    ...
    return description += ContainerProps::containerHumanNames().value(container) + " | " +
                          server.value(config_key::hostName).toString();
}
...
const int ServersModel::getServersCount() { return m_servers.count(); }
...
int ServersModel::getProcessedServerIndex() { return m_processedServerIndex; }

const ServerCredentials ServersModel::getProcessedServerCredentials() {
    return serverCredentials(m_processedServerIndex);
}

const ServerCredentials ServersModel::getServerCredentials(const int index) { return serverCredentials(index); }

bool ServersModel::isDefaultServerCurrentlyProcessed() { return m_defaultServerIndex == m_processedServerIndex; }
...
void ServersModel::editServer(const QJsonObject &server, const int serverIndex) {
    m_servers.replace(serverIndex, server);
...
void ServersModel::removeServer() {
    beginResetModel();
    if (m_processedServerIndex < m_servers.size())
        m_servers.removeAt(m_processedServerIndex);

    if (m_defaultServerIndex == m_processedServerIndex) {
        setDefaultServerIndex(0);
    } else if (m_defaultServerIndex > m_processedServerIndex) {
        setDefaultServerIndex(m_defaultServerIndex - 1);
    }
...
QJsonObject ServersModel::getServerConfig(const int serverIndex) { return m_servers.at(serverIndex).toObject(); }

QMap<DockerContainer, QJsonObject> ServersModel::containers(int serverIndex) {
    const QJsonArray &containers = m_servers.at(serverIndex).toObject().value(config_key::containers).toArray();

    QMap<DockerContainer, QJsonObject> containersMap;
    for (const QJsonValue &val: containers) {
        containersMap.insert(
                ContainerProps::containerFromString(val.toObject().value(config_key::container).toString()),
                val.toObject());
    }
...
QJsonObject ServersModel::containerConfig(int serverIndex, DockerContainer container) {
...
void ServersModel::reloadDefaultServerContainerConfig() {
    ...
    auto config = containerConfig(m_defaultServerIndex, container);
    for (auto i = 0; i < containers.size(); i++) {
        auto c = ContainerProps::containerFromString(
                containers.at(i).toObject().value(config_key::container).toString());
        if (c == container) {
...
ErrorCode ServersModel::removeAllContainers(const QSharedPointer<ServerController> &serverController) {
    ...
    ErrorCode errorCode = serverController->removeAllContainers(serverCredentials(m_processedServerIndex));
...
ErrorCode ServersModel::removeContainer(const QSharedPointer<ServerController> &serverController,
    ...
    if (errorCode == ErrorCode::NoError) {
        ...
        for (auto it = containers.begin(); it != containers.end(); it++) {
            if (it->toObject().value(config_key::container).toString() ==
                ContainerProps::containerToString(dockerContainer)) {
                containers.erase(it);
                break;
            }
        }
        ...
        if (defaultContainer == containerIndex) {
            if (containers.empty()) {
                ...
                defaultContainer = ContainerProps::containerFromString(
                        containers.begin()->toObject().value(config_key::container).toString());
            }
...
void ServersModel::clearCachedProfile(const DockerContainer container) {
    clearLastConnectionConfig(m_processedServerIndex, container);
    m_servers.replace(m_processedServerIndex, m_servers.at(m_processedServerIndex).toObject());
...
bool ServersModel::isAmneziaDnsContainerInstalled(const int serverIndex) const {
    ...
    for (auto it = containers.begin(); it != containers.end(); it++) {
        if (it->toObject().value(config_key::container).toString() ==
            ContainerProps::containerToString(DockerContainer::Dns)) {
            return true;
        }
    }
    return false;
...
QPair<QString, QString> ServersModel::getDnsPair(int serverIndex) {
    ...
    for (const QJsonValue &container: containers) {
        if (ContainerProps::containerFromString(container.toObject().value(config_key::container).toString()) ==
            DockerContainer::Dns) {
...
bool ServersModel::isServerFromApiAlreadyExists(const QString &userCountryCode, const QString &serviceType,
...
bool ServersModel::isDefaultServerDefaultContainerHasSplitTunneling() {
    ...
    for (auto i = 0; i < containers.size(); i++) {
        ...
        if (defaultContainer == DockerContainer::Awg || defaultContainer == DockerContainer::WireGuard) {
            QJsonObject serverProtocolConfig =
            ...
            return (clientProtocolConfigString.contains("AllowedIPs") &&
                    !clientProtocolConfigString.contains("AllowedIPs = 0.0.0.0/0, ::/0")) ||
            ...
            auto serverProtocolConfig =
                    container.value(ContainerProps::containerTypeToString(DockerContainer::OpenVpn)).toObject();
            QString clientProtocolConfigString = serverProtocolConfig.value(config_key::last_config).toString();
