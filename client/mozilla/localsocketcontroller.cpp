...
LocalSocketController::LocalSocketController() {
    ...
    connect(m_socket, &QLocalSocket::connected, this, &LocalSocketController::daemonConnected);
    connect(m_socket, &QLocalSocket::disconnected, this, &LocalSocketController::disconnected);
    connect(m_socket, &QLocalSocket::errorOccurred, this, &LocalSocketController::errorOccurred);
    connect(m_socket, &QLocalSocket::readyRead, this, &LocalSocketController::readData);

    m_initializingTimer.setSingleShot(true);
    connect(&m_initializingTimer, &QTimer::timeout, this, &LocalSocketController::initializeInternal);
}

LocalSocketController::~LocalSocketController() { MZ_COUNT_DTOR(LocalSocketController); }

void LocalSocketController::errorOccurred(QLocalSocket::LocalSocketError error) {
...
void LocalSocketController::activate(const QJsonObject &rawConfig) {
    ...
    if (protocolName == amnezia::config_key::awg) {
        ...
        json.insert(amnezia::config_key::responsePacketJunkSize,
                    wgConfig.value(amnezia::config_key::responsePacketJunkSize));
        json.insert(amnezia::config_key::initPacketMagicHeader,
                    wgConfig.value(amnezia::config_key::initPacketMagicHeader));
        json.insert(amnezia::config_key::responsePacketMagicHeader,
                    wgConfig.value(amnezia::config_key::responsePacketMagicHeader));
        json.insert(amnezia::config_key::underloadPacketMagicHeader,
                    wgConfig.value(amnezia::config_key::underloadPacketMagicHeader));
        json.insert(amnezia::config_key::transportPacketMagicHeader,
                    wgConfig.value(amnezia::config_key::transportPacketMagicHeader));
...
void LocalSocketController::getBackendLogs(std::function<void(const QString &)> &&a_callback) {
...
void LocalSocketController::parseCommand(const QByteArray &command) {
    ...
    if (type == "status") {
        ...
        emit statusUpdated(serverIpv4Gateway.toString(), deviceIpv4Address.toString(), txBytes.toDouble(),
                           rxBytes.toDouble());
        return;
    }
