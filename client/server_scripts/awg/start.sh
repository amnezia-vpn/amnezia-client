...
# awg-quick runs `set -e` with a teardown trap, so a failed `ip -6 address add`
# on an IPv6-disabled host aborts the whole tunnel (including IPv4). If the host
# has no usable IPv6, drop the v6 address so awg-quick won't try to add it.
if [ -f /proc/net/if_inet6 ] && [ "$(cat /proc/sys/net/ipv6/conf/all/disable_ipv6 2>/dev/null)" = "0" ]; then
    IPV6_OK=true
else
    IPV6_OK=false
    sed -i '/^Address[[:space:]]*=/s/,.*$//' /opt/amnezia/awg/awg0.conf
...
if [ "$IPV6_OK" = true ] && command -v ip6tables >/dev/null 2>&1; then
    ip6tables -A INPUT -i awg0 -j ACCEPT || true
    ip6tables -A FORWARD -i awg0 -j ACCEPT || true
    ip6tables -A OUTPUT -o awg0 -j ACCEPT || true

    ip6tables -A FORWARD -i awg0 -o eth0 -s $AWG_SUBNET_IPV6/$WIREGUARD_SUBNET_IPV6_CIDR -j ACCEPT || true
    ip6tables -A FORWARD -i awg0 -o eth1 -s $AWG_SUBNET_IPV6/$WIREGUARD_SUBNET_IPV6_CIDR -j ACCEPT || true
