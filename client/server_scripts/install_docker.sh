...
elif which pacman > /dev/null 2>&1; then pm=$(which pacman); docker_pkg="docker"; dist="archlinux";\
...
if ! command -v docker > /dev/null 2>&1; then sudo $pm update -yq; sudo $pm install -yq $docker_pkg;\
  if [ "$dist" = "archlinux" ] || [ "$dist" = "fedora" ] || [ "$dist" = "centos" ] || [ "$dist" = "debian" ]; then sudo systemctl enable docker && sudo systemctl start docker; fi;\
fi;\
if [ "$dist" = "debian" ]; then \
