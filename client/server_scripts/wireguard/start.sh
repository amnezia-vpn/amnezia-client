...
if [ -f /proc/net/if_inet6 ] && [ "$(cat /proc/sys/net/ipv6/conf/all/disable_ipv6 2>/dev/null)" = "0" ]; then
    ...
    IPV6_OK=false
    sed -i '/^Address[[:space:]]*=/s/,.*$//' /opt/amnezia/wireguard/wg0.conf
    echo "Host IPv6 unavailable, starting WireGuard IPv4-only"
fi
...
if [ "$IPV6_OK" = true ] && command -v ip6tables >/dev/null 2>&1; then
    ...
    ip6tables -A FORWARD -i wg0 -o eth0 -s $WIREGUARD_SUBNET_IPV6/$WIREGUARD_SUBNET_IPV6_CIDR -j ACCEPT || true
    ip6tables -A FORWARD -i wg0 -o eth1 -s $WIREGUARD_SUBNET_IPV6/$WIREGUARD_SUBNET_IPV6_CIDR -j ACCEPT || true
    ip6tables -A FORWARD -m state --state ESTABLISHED,RELATED -j ACCEPT || true

    ip6tables -t nat -A POSTROUTING -s $WIREGUARD_SUBNET_IPV6/$WIREGUARD_SUBNET_IPV6_CIDR -o eth0 -j MASQUERADE || true
    ip6tables -t nat -A POSTROUTING -s $WIREGUARD_SUBNET_IPV6/$WIREGUARD_SUBNET_IPV6_CIDR -o eth1 -j MASQUERADE || true
fi
