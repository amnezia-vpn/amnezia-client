...
WireGuardProtocolConfig::WireGuardProtocolConfig(const QString &protocolName, int port, TransportProto transportProto) : ProtocolConfig(protocolName)
{
...
WireGuardProtocolConfig::WireGuardProtocolConfig(const WireGuardProtocolConfig &other) : ProtocolConfig(other.protocolName)
{
...
ScriptVars WireGuardProtocolConfig::getScriptVars() const
{
    ScriptVars vars;

    if (!serverProtocolConfig.subnetAddress.isEmpty()) {
        vars.append({ "$WIREGUARD_SUBNET_IP", serverProtocolConfig.subnetAddress });
    }
    if (!serverProtocolConfig.port.isEmpty()) {
        vars.append({ "$WIREGUARD_SERVER_PORT", serverProtocolConfig.port });
    }
