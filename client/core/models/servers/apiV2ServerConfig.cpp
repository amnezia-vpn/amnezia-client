...
ApiV2ServerConfig::ApiV2ServerConfig() : ServerConfig()
{
    type = ServerConfigType::ApiV2;
}
...
ApiV2ServerConfig::ApiV2ServerConfig(const QJsonObject &serverConfigObject) : ServerConfig(serverConfigObject)
{
    ...
    apiConfig.subscription.endDate = subscriptionObject.value("end_date").toString();
...
ApiV2ServerConfig::ApiV2ServerConfig(const ApiV2ServerConfig &other)
    : ServerConfig(other),
      name(other.name),
...
QJsonObject ApiV2ServerConfig::toJson() const
{
    ...
    if (!apiConfig.subscription.endDate.isEmpty()) {
        ...
        subscriptionObject["end_date"] = apiConfig.subscription.endDate;
...
bool ApiV2ServerConfig::isPremium()
{
    return apiConfig.serviceType == servicePremium || apiConfig.serviceType == serviceExternalPremium;
}

QJsonObject apiv2::AuthData::toJson()
{
    QJsonObject authDataJson;
    authDataJson["api_key"] = apiKey;
    return authDataJson;
}
