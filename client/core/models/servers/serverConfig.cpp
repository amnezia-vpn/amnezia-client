...
ServerConfig::ServerConfig(const QJsonObject &serverConfigObject)
{
    ...
    defaultContainerName = serverConfigObject.value(config_key::defaultContainer).toString();
    ...
    for (const auto &container : std::as_const(containers)) {
        ...
        for (const auto &protocol : protocols) {
            ...
            case Proto::TorWebSite: {
                containerConfig.protocolConfigs.insert(protocolName,
                                                       QSharedPointer<TorProtocolConfig>::create(protocolConfigObject, protocolName));
                break;
            }
            case Proto::Ikev2: {
                containerConfig.protocolConfigs.insert(protocolName,
                                                       QSharedPointer<IpsecProtocolConfig>::create(protocolConfigObject, protocolName));
                break;
            }
            case Proto::Sftp: {
                containerConfig.protocolConfigs.insert(protocolName,
                                                       QSharedPointer<SftpProtocolConfig>::create(protocolConfigObject, protocolName));
                break;
            }
            case Proto::Socks5Proxy: {
                containerConfig.protocolConfigs.insert(
                        protocolName, QSharedPointer<Socks5ProxyProtocolConfig>::create(protocolConfigObject, protocolName));
                break;
            }
...
ServerConfig::ServerConfig(const ServerConfig &other)
    : type(other.type),
      ...
      defaultContainerType(other.defaultContainerType),
      nameOverriddenByUser(other.nameOverriddenByUser),
      crc(other.crc),
      containerConfigs(other.containerConfigs)
{
}
...
QJsonObject ServerConfig::toJson() const
{
    ...
    if (!defaultContainerName.isEmpty()) {
        json[config_key::defaultContainer] = defaultContainerName;
    }
