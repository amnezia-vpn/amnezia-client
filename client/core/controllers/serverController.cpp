...
ErrorCode ServerController::runContainerScript(const ServerCredentials &credentials, DockerContainer container, QString script,
                                               ...
                                               const std::function<ErrorCode(const QString &, libssh::Client &)> &cbReadStdErr)
{
    QString fileName = "/opt/fblink/" + Utils::getRandomString(16) + ".sh";
...
{
    return runScript(credentials, fblink::scriptData(SharedScriptType::remove_all_containers));
}

ErrorCode ServerController::removeContainer(const ServerCredentials &credentials, DockerContainer container)
{
    return runScript(credentials,
                     replaceVars(fblink::scriptData(SharedScriptType::remove_container), genVarsForScript(credentials, container)));
}
...
ErrorCode ServerController::installDockerWorker(const ServerCredentials &credentials, DockerContainer container)
{
    ...
    ErrorCode error =
            runScript(credentials, replaceVars(fblink::scriptData(SharedScriptType::install_docker), genVarsForScript(credentials)),
...
ErrorCode ServerController::prepareHostWorker(const ServerCredentials &credentials, DockerContainer container, const QJsonObject &config)
{
    ...
    return runScript(credentials, replaceVars(fblink::scriptData(SharedScriptType::prepare_host), genVarsForScript(credentials, container)));
}
...
{
    QString dockerFilePath = fblink::server::getDockerfileFolder(container) + "/Dockerfile";
    ...
    errorCode = uploadFileToHost(credentials, fblink::scriptData(ProtocolScriptType::dockerfile, container).toUtf8(), dockerFilePath);
    ...
    ErrorCode error =
            runScript(credentials,
                      replaceVars(fblink::scriptData(SharedScriptType::build_container), genVarsForScript(credentials, container, config)),
                      cbReadStdOut, cbReadStdErr);
...
ErrorCode ServerController::runContainerWorker(const ServerCredentials &credentials, DockerContainer container, QJsonObject &config)
{
    ...
    ErrorCode e = runScript(credentials,
                            replaceVars(fblink::scriptData(ProtocolScriptType::run_container, container),
...
ErrorCode ServerController::configureContainerWorker(const ServerCredentials &credentials, DockerContainer container, QJsonObject &config)
{
    ...
    ErrorCode e = runContainerScript(credentials, container,
                                     replaceVars(fblink::scriptData(ProtocolScriptType::configure_container, container),
                                                 genVarsForScript(credentials, container, config)),
                                     cbReadStdOut, cbReadStdErr);
...
{
    QString script = fblink::scriptData(ProtocolScriptType::container_startup, container);
    ...
    return runScript(credentials,
                     replaceVars("sudo docker exec -d $CONTAINER_NAME sh -c \"chmod a+x /opt/fblink/start.sh && "
                                 "/opt/fblink/start.sh\"",
                                 genVarsForScript(credentials, container, config)));
}
                                                          ...
                                                          const QJsonObject &config)
{
    ...
    const QJsonObject &fblinkWireguarConfig = config.value(ProtocolProps::protoToString(Proto::Awg)).toObject();
    ...
    vars.append({ { "$DOCKERFILE_FOLDER", "/opt/fblink/" + ContainerProps::containerToString(container) } });
    ...
    // FBLink wireguard vars
    vars.append({ { "$AWG_SUBNET_IP",
                    fblinkWireguarConfig.value(config_key::subnet_address).toString(protocols::wireguard::defaultSubnetAddress) } });
    vars.append({ { "$AWG_SERVER_PORT", fblinkWireguarConfig.value(config_key::port).toString(protocols::awg::defaultPort) } });

    vars.append({ { "$JUNK_PACKET_COUNT", fblinkWireguarConfig.value(config_key::junkPacketCount).toString() } });
    vars.append({ { "$JUNK_PACKET_MIN_SIZE", fblinkWireguarConfig.value(config_key::junkPacketMinSize).toString() } });
    vars.append({ { "$JUNK_PACKET_MAX_SIZE", fblinkWireguarConfig.value(config_key::junkPacketMaxSize).toString() } });
    vars.append({ { "$INIT_PACKET_JUNK_SIZE", fblinkWireguarConfig.value(config_key::initPacketJunkSize).toString() } });
    vars.append({ { "$RESPONSE_PACKET_JUNK_SIZE", fblinkWireguarConfig.value(config_key::responsePacketJunkSize).toString() } });
    vars.append({ { "$INIT_PACKET_MAGIC_HEADER", fblinkWireguarConfig.value(config_key::initPacketMagicHeader).toString() } });
    vars.append({ { "$RESPONSE_PACKET_MAGIC_HEADER", fblinkWireguarConfig.value(config_key::responsePacketMagicHeader).toString() } });
    vars.append({ { "$UNDERLOAD_PACKET_MAGIC_HEADER", fblinkWireguarConfig.value(config_key::underloadPacketMagicHeader).toString() } });
    vars.append({ { "$TRANSPORT_PACKET_MAGIC_HEADER", fblinkWireguarConfig.value(config_key::transportPacketMagicHeader).toString() } });

    vars.append({ { "$COOKIE_REPLY_PACKET_JUNK_SIZE", fblinkWireguarConfig.value(config_key::cookieReplyPacketJunkSize).toString() } });
    vars.append({ { "$TRANSPORT_PACKET_JUNK_SIZE", fblinkWireguarConfig.value(config_key::transportPacketJunkSize).toString() } });
    vars.append({ { "$SPECIAL_JUNK_1", fblinkWireguarConfig.value(config_key::specialJunk1).toString() } });
    vars.append({ { "$SPECIAL_JUNK_2", fblinkWireguarConfig.value(config_key::specialJunk2).toString() } });
    vars.append({ { "$SPECIAL_JUNK_3", fblinkWireguarConfig.value(config_key::specialJunk3).toString() } });
    vars.append({ { "$SPECIAL_JUNK_4", fblinkWireguarConfig.value(config_key::specialJunk4).toString() } });
    vars.append({ { "$SPECIAL_JUNK_5", fblinkWireguarConfig.value(config_key::specialJunk5).toString() } });
...
QString ServerController::checkSshConnection(const ServerCredentials &credentials, ErrorCode &errorCode)
{
    ...
    errorCode = runScript(credentials, fblink::scriptData(SharedScriptType::check_connection), cbReadStdOut, cbReadStdErr);
...
{
    return runScript(credentials, replaceVars(fblink::scriptData(SharedScriptType::setup_host_firewall), genVarsForScript(credentials)));
}
...
ErrorCode ServerController::isUserInSudo(const ServerCredentials &credentials, DockerContainer container)
{
    ...
    const QString scriptData = fblink::scriptData(SharedScriptType::check_user_in_sudo);
...
ErrorCode ServerController::isServerDpkgBusy(const ServerCredentials &credentials, DockerContainer container)
{
    ...
    QFuture<ErrorCode> future = QtConcurrent::run([this, &stdOut, &cbReadStdOut, &cbReadStdErr, &credentials]() {
        ...
        for (int i = 0; i < 30; ++i) {
            ...
            stdOut.clear();
            runScript(credentials, replaceVars(fblink::scriptData(SharedScriptType::check_server_is_busy), genVarsForScript(credentials)),
                      cbReadStdOut, cbReadStdErr);
