                                              ...
                                              ErrorCode &errorCode)
{
    ...
    QString config_template = m_sshSession->replaceVars(amnezia::scriptData(ProtocolScriptType::xray_template, container), vars);

    if (config_template.isEmpty()) {
    ...
    if (!config_template.contains("$XRAY_CLIENT_ID") ||
        !config_template.contains("$XRAY_PUBLIC_KEY") ||
        !config_template.contains("$XRAY_SHORT_ID")) {

        logger.error() << "Config template missing required variables:"
                       << "XRAY_CLIENT_ID:" << !config_template.contains("$XRAY_CLIENT_ID")
                       << "XRAY_PUBLIC_KEY:" << !config_template.contains("$XRAY_PUBLIC_KEY")
                       << "XRAY_SHORT_ID:" << !config_template.contains("$XRAY_SHORT_ID");
        errorCode = ErrorCode::InternalError;
        return XrayProtocolConfig{};
    }

    config_template.replace("$XRAY_CLIENT_ID", xrayClientId);
    config_template.replace("$XRAY_PUBLIC_KEY", xrayPublicKey);
    config_template.replace("$XRAY_SHORT_ID", xrayShortId);
