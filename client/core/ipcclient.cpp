...
IpcClientEvents *IpcClientEvents::m_instance = nullptr;

IpcClientEvents *IpcClientEvents::Instance() {
    if (!m_instance) {
        m_instance = new IpcClientEvents();
    }
...
IpcClient::IpcClient(QObject *parent) : QObject(parent) { m_uuid = QUuid::createUuid(); }
...
IpcClient *IpcClient::Reinitialize(QObject *parent) {
  if (m_instance && m_instance->m_ipcClient) {
    ...
    if (!m_instance->isSocketConnected()) {
        ...
        connect(m_instance->Interface().get(), &IpcInterfaceReplica::splitTunnelingFailed, IpcClientEvents::Instance(),
                &IpcClientEvents::splitTunnelingFailed);
    }
...
bool IpcClient::init(IpcClient *instance) {
    ...
    qDebug() << "IpcClient::isReplicaValid: " << Instance()->m_ipcClient->isReplicaValid()
