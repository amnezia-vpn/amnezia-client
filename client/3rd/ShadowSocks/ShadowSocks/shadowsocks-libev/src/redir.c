...
#include "common.h"
#include "redir.h"
#include "../../libev/arm64/include/ev.h"
...
getdestaddr(int fd, struct sockaddr_storage *destaddr)
{
    ...
    if (tcp_tproxy) {
        ...
        error = getsockopt(fd, 10/*SOL_IPV6*/, IP6T_SO_ORIGINAL_DST, destaddr, &socklen);
        if (error) { // Didn't find a proper way to detect IP version.
            error = getsockopt(fd, 20/*SOL_IP*/, 80/*SO_ORIGINAL_DST*/, destaddr, &socklen);
        }
    }
...
int
rdr_create_and_bind(const char *addr, const char *port)
{
...
accept_cb(EV_P_ ev_io *w, int revents)
{
    ...
    setsockopt(remotefd, SOL_TCP, TCP_KEEPALIVE, (void *)&keepIdle, sizeof(keepIdle));
...
main(int argc, char **argv)
{
    ...
    if (conf_path != NULL) {
        ...
        if (redir_fast_open == 0) {
            redir_fast_open = conf->fast_open;
        }
#ifdef HAVE_SETRLIMIT
        if (nofile == 0) {
            nofile = conf->nofile;
...
//        set_nofile(nofile);
    ...
    do {
        ...
        if (mode != UDP_ONLY) {
            // Setup socket
            int listenfd;
            listenfd = rdr_create_and_bind(local_addr, local_port);
            if (listenfd == -1) {
                FATAL("bind() error");
            }
