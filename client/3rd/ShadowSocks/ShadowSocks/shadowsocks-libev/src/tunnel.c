...
int
create_and_bind_tun(const char *addr, const char *port)
{
 ...
 main(int argc, char **argv)
{
    ...
    if (conf_path != NULL) {
        ...
        if (tun_reuse_port == 0) {
            tun_reuse_port = conf->reuse_port;
        }
        if (tun_fast_open == 0) {
            tun_fast_open = conf->fast_open;
        }
#ifdef HAVE_SETRLIMIT
        if (nofile == 0) {
            nofile = conf->nofile;
...
//        set_nofile(nofile);
    ...
    if (mode != UDP_ONLY) {
        // Setup socket
        int listenfd;
        listenfd = create_and_bind_tun(local_addr, local_port);
        if (listenfd == -1) {
            FATAL("bind() error");
        }
