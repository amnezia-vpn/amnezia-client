...
constexpr const char* TMP_PATH = "/tmp/fblink.socket";
constexpr const char* VAR_PATH = "/var/run/fblink/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("fblink")) {
    logger.debug() << "/var/run/fblink seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("fblink")) {
    logger.warning() << "Failed to create /var/run/fblink";
    return TMP_PATH;
  }

  if (chmod("/var/run/fblink", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/fblink";
    return TMP_PATH;
  }
