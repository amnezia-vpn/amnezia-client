...
// Golden Фазы 1: воспроизводим крипто-конвейер тела запроса на фиксированных key/iv/salt и
// сверяем то, что детерминировано (json_keys, api_payload) — байт-в-байт; key_payload (RSA,
// рандомный паддинг) — через round-trip. Сборка тела здесь инлайн; в Фазе 2 её формализует
// src/protocol/request_builder.
int main()
{
    namespace k = protocol::keys;

    // Фиксированные значения (совпадают с tests/.../fixtures и независимым openssl-расчётом).
    ...
    // --- json_keys: байт-в-байт (отсортированные ключи + Qt-Indented) -----
    util::Json keysJson;
    ...
    // --- api_payload: байт-в-байт (детерминирован) ------------------------
    ...
    // --- key_payload: round-trip (RSA-паддинг недетерминирован) -----------
    ...
    // расшифровываем как сервер: base64decode → RSA-decrypt → должны получить ровно json_keys
    ...
    // --- outer body: структурно (api_payload точно, key_payload round-trip) ---
    util::Json body;
    ...
    // расшифровка ответа тем же ключом/iv (как сервер шлёт ответ) -----------
    {
