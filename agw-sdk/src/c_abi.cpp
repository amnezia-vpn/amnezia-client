...
agw_client *agw_client_create(const agw_config *config)
{
    ...
    if (config->request_timeout_msecs > 0) cfg.requestTimeoutMsecs = config->request_timeout_msecs;
    if (config->proxy_health_timeout_msecs > 0) cfg.proxyHealthTimeoutMsecs = config->proxy_health_timeout_msecs;
    if (config->proxy_storage_timeout_msecs > 0) cfg.proxyStorageTimeoutMsecs = config->proxy_storage_timeout_msecs;
    if (config->thread_pool_size > 0) cfg.threadPoolSize = config->thread_pool_size;
    ...
    if (config->log != nullptr) {
        ...
        cfg.log = [fn, ud](agw::LogLevel level, const std::string &msg) {
            fn(static_cast<int>(level), msg.c_str(), ud);
        };
    }
...
agw_response agw_client_post(agw_client *client, const char *endpoint, const char *payload,
...
void agw_client_post_async(agw_client *client, const char *endpoint, const char *payload,
                           const char *service_type, const char *user_country_code,
                           agw_post_callback callback, void *user_data, agw_cancel_token *cancel_token)
{
