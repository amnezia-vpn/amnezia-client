...
int IpcServer::createPrivilegedProcess() {
    ...
    QObject::connect(
            pd.serverNode.data(), &QRemoteObjectHost::error, this,
...
bool IpcServer::createTun(const QString &dev, const QString &subnet) { return Router::createTun(dev, subnet); }
...
void IpcServer::connectionEstablished(const QUuid &uniqueId) { m_pendingCleanClose.insert(uniqueId); }

void IpcServer::connectionClose(const QUuid &uniqueId) { m_pendingCleanClose.remove(uniqueId); }

bool IpcServer::hasPendingConnectionClose() { return !m_pendingCleanClose.isEmpty(); }
